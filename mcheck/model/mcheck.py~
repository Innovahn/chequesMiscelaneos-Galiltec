...
class mcheck(osv.Model):
	...
	def cancel_voucher(self, cr, uid, ids, context=None):
		...
		for mcheck in self.browse(cr,uid,ids,context=context):
			...
			selected_cancel_date=datetime.now()#selected date for tha cancelation 
	...
	def onchange_amount_line(self,cr,uid,ids,total,context=None):	
		total_line = 0
		dec_prec = 0	
		for m in self.browse(cr,uid,ids,context=context):
			for lines in m.mcheck_ids:
				total_line+=lines.amount
			if total_line>total:
				raise osv.except_osv(_('to much for this line'),_("Change total or put less in ammount") )
		...
		for mcheck in self.browse(cr,uid,ids,context=context):
			if len(mcheck.mcheck_ids) > 0:
				...
				if ((totald-totalc) > 0 and flag==True):
					...
					try:
						cr.execute("UPDATE ir_sequence SET number_next=number_next+number_increment WHERE id=%s ", (mcheck.journal_id.sequence_id.id,))
					except ValueError:
					...
					return self.write(cr, uid, ids, {'state':'validated','number':name,'move_id':move_id, 'actual_comp_rate': currency_rate,'actual_sec_curr_rate': select_journal_currency_rate }, context=context)
				else:
	...
	def onchange_journal(self,cr,uid,ids,journalid,context=None):
		...
		if diario.sequence_id:
			...
			#c = dict(context)
			obj_seq = seq_model.browse(cr, uid, seq_model.search(cr,uid,[('id','=',diario.sequence_id.id)],context=None))
	...
	_sql_constraints={
	('unique_temp_code','unique(temporal_code)','The Temporal code code must be unique!')
	}
...
class mcheck_name(osv.Model):
	...
	def _get_lineas_tot(self, cr, uid, ids, field, arg, context=None):
		result={}
	...
	_columns = {
		...
		'amount_line':fields.function(_get_lineas_tot,type='float',string='Total',store=False),
