...
class banks_deposits(osv.Model):
	...
	def update_state_draf(self, cr, uid, ids,journal_id,doc_type, context=None):
		...
		mchecks_obj=mchecks_model.browse(cr,uid,mchecks_model.search(cr,uid,[('state','=','draft' ),('doc_type','=',doc_type)],context=None),context=context)
	...
	_columns={
		...
		'name':fields.text('Memo', required=False),
	...
	@api.multi
		...
		#default['number'] = self.journal_number(self, self.id)
		default['state'] ='draft'
		...
		for line in self.mcheck_ids:
			a = self.env['banks.deposit_name'].create({'mcheck_id': encabezado.id, 'account_id': line.account_id.id, 'name':line.name, 'amount': line.amount, 'chqmanalitics': line.chqmanalitics.id, 'type':line.type})
